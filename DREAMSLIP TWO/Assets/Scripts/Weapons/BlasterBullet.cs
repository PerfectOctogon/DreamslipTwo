...
public class BlasterBullet : MonoBehaviour
{
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if (other.CompareTag("Player") || other.CompareTag("Triggers")) { return; }
        foreach (RaycastHit hit in Physics.SphereCastAll(transform.position, bulletExposionRange, transform.forward)) {
            //print(hit.collider.name);
            if (hit.collider.gameObject.CompareTag("Enemy")) { hit.collider.gameObject.GetComponent<EnemyAI>().Damage(explosionDamage); print("Explosion!"); }
        }
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        {
            print("Hit enemy with blaster shot!");
        }*/
        //DestroyBullet(transform.position);
    }
